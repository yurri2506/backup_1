...
################################################################################
# V4 BENCHMARK SCRIPT - Combined SABV4 + LMTR4 + SP1 Proving
# 
...
LOG_DIR_SINGLE="/home/ubuntu/thanhhuyen/logs/sabv4_lmtr4"
LOG_DIR_SEQ="/home/ubuntu/thanhhuyen/logs/v4_seq"
...
run_single_test() {
    local N=${2:-1}
    local SESSION_NAME="sabv4_lmtr4_ppgen"
    
    echo "🚀 Starting V4 SINGLE Test - N=$N with tmux monitoring"
    ...
    tmux send-keys -t "$SESSION_NAME":0.0 "/usr/bin/time -v RUST_LOG=sp1_sdk=debug,sp1=debug SP1_PROVER=cpu SP1_CARGO_PROVE_PATH=/home/ubuntu/.sp1/bin/cargo-prove cargo run --release -p pessimistic-proof-test-suite --bin ppgen_sabv_lmtr4 -- --n-exits $N --validator-nodes 5 --proof-dir $PROOF_DIR 2>&1 | tee $LOG_DIR_SINGLE/v4_n${N}_\$(date +%Y%m%d_%H%M%S).log" C-m
    ...
    tmux send-keys -t "$SESSION_NAME":0.2 "tail -f $LOG_DIR_SINGLE/v4_n${N}_*.log 2>/dev/null || echo 'Waiting for log file...'" C-m
    ...
    echo "✅ V4 SINGLE Test tmux session created: $SESSION_NAME"
...
run_sequential_benchmark() {
    ...
    for N in "${N_VALUES[@]}"; do
        ...
        mkdir -p "$LOG_DIR_SEQ"
        LOG_FILE="$LOG_DIR_SEQ/v4_n${N}.log"
        START_TIME=$(date +%s)
        
        # Run with CPU/RAM monitoring in background
        (
            while kill -0 $$ 2>/dev/null; do
                ps aux | grep ppgen_sabv_lmtr4 | grep -v grep | awk '{print $3,$4}' >> "$LOG_DIR_SEQ/v4_n${N}_system.csv" 2>/dev/null || true
                sleep 5
            done
        ) &
        MONITOR_PID=$!
...
run_all() {
    echo "╔════════════════════════════════════════════════════════════════════════╗"
    echo "║     V4 COMBINED BENCHMARK: Single Test + Sequential Batch              ║"
    echo "╚════════════════════════════════════════════════════════════════════════╝"
    echo ""
    ...
    echo ""
    echo "📝 Note: Single test running in tmux session 'sabv4_lmtr4_ppgen'"
    echo "   Attach anytime: tmux attach -t sabv4_lmtr4_ppgen"
    echo ""
    ...
    if [[ "$WAIT_SINGLE" == "y" ]] || [[ "$WAIT_SINGLE" == "Y" ]]; then
        ...
        while tmux has-session -t "sabv4_lmtr4_ppgen" 2>/dev/null; do
            sleep 10
        done
    ...
    echo ""
    echo "╔════════════════════════════════════════════════════════════════════════╗"
    echo "║     ✅ ALL V4 BENCHMARKS COMPLETED!                                     ║"
    echo "╚════════════════════════════════════════════════════════════════════════╝"
    echo ""
    echo "📊 Results:"
