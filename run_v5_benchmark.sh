...
################################################################################
# V5 BENCHMARK SCRIPT - Run N=1, 10, 100, 200, 500, 700 (lặp lại 5 lần)
# 
...
# Usage:
#   ./run_v5_benchmark.sh
#   hoặc trong tmux: tmux new-session -s v5_benchmark 'bash run_v5_benchmark.sh'
################################################################################
...
TARGET_RUNS=5
VALIDATORS=5
TMUX_SESSION="v5_benchmark"

echo "═══════════════════════════════════════════════════════════════"
echo "🚀 V5 BENCHMARK: N=1, 10, 100, 200, 500, 700 (lặp lại 5 lần)"
echo "═══════════════════════════════════════════════════════════════"
echo ""
echo "📋 Configuration:"
...
monitor_process() {
    ...
    while true; do
        ...
        if [ -z "$pid" ]; then
            # Fallback 2: tìm bằng --n-exits $n và proof dir v5
            pid=$(ps aux | grep "ppgen_sabv_lmtr5" | grep "--n-exits $n" | grep "/proofs/v5" | grep -v grep | awk '{print $2}' | head -1)
        fi
        ...
        if [ -n "$pid" ]; then
            ...
            if ! ps aux | grep "ppgen_sabv_lmtr5" | grep "/proofs/v5" | grep -v grep > /dev/null; then
...
# Để chạy trong tmux, dùng: tmux new-session -s v5_benchmark 'bash run_v5_benchmark.sh'
...
for run in $(seq 1 $TARGET_RUNS); do
    ...
    for N in "${N_VALUES[@]}"; do
        ...
        LOG_FILE="$LOG_DIR/v5_n${N}_run${run}_$(date +%Y%m%d_%H%M%S).log"
        ...
        if [ $EXIT_CODE -eq 0 ]; then
            ...
            if [ -f "$PROOF_SUBDIR/v5_proof_n${N}.json" ] || [ -f "$PROOF_SUBDIR/proof.bin" ]; then
                if [ -f "$PROOF_SUBDIR/v5_proof_n${N}.json" ]; then
                    ...
                    echo "   ✅ Proof file: v5_proof_n${N}.json ($PROOF_SIZE)"
                fi
...
echo "   hoặc: ls -lht $LOG_DIR/v5_n*_run*.log"
echo ""
echo "To check results:"
echo "   grep -E 'SUCCESS|FAILED|PASSED|signal 9' $LOG_DIR/v5_n*_run*.log"
echo ""
echo "✅ TMUX session '$TMUX_SESSION' được GIỮ LẠI - Attach: tmux attach -t $TMUX_SESSION"
echo "💡 Log files: ls -lht $LOG_DIR/v5_n*_run*.log"
