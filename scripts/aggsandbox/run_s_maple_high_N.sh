...
# Script chạy S-MAPLE với N=200, 500, 700, mỗi N chạy 5 lần
# Usage: ./run_s_maple_high_N.sh [NUM_RUNS]
# Default: NUM_RUNS=5
...
EXIT_COUNTS="${EXIT_COUNTS:-200 500 700}"
NUM_RUNS="${1:-5}"

echo "═══════════════════════════════════════════════════════════"
echo "🧬 AGGSANDBOX EXPERIMENTS: S-MAPLE - HIGH N VALUES (200, 500, 700) - $NUM_RUNS RUNS EACH"
echo "═══════════════════════════════════════════════════════════"
echo ""
...
if [ ! -f "$SUMMARY_FILE" ]; then
  ...
  echo "" >> "$SUMMARY_FILE"
  echo "# Starting S-MAPLE HIGH N experiments (200, 500, 700) with $NUM_RUNS runs per N - $(date)" >> "$SUMMARY_FILE"
fi

echo ""
echo "═══════════════════════════════════════════════════════════"
echo "🧬 STARTING S-MAPLE HIGH N EXPERIMENTS - $NUM_RUNS RUNS PER N"
echo "═══════════════════════════════════════════════════════════"
echo "" | tee -a "$LOG_DIR/run.log"
echo "[$(date +%Y-%m-%d_%H:%M:%S)] Starting S-MAPLE (SABV5+LMTR4) HIGH N experiments" | tee -a "$LOG_DIR/run.log"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "✅ ALL S-MAPLE HIGH N EXPERIMENTS COMPLETED"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "[$(date +%Y-%m-%d_%H:%M:%S)] All S-MAPLE HIGH N experiments completed!" | tee -a "$LOG_DIR/run.log"
...
echo ""
echo "📈 Quick stats:"
grep "^s_maple,N=" "$SUMMARY_FILE" 2>/dev/null | grep -E "N=200|N=500|N=700" | tail -20 || echo "   (No results yet)"
echo ""
