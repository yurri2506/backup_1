...
tmux send-keys -t "$SESSION_NAME":0.3 "for N in 70 200 400 700; do echo \"[BASELINE] Running N=\$N\" | tee -a $LOG_DIR/run.log; /usr/bin/time -v cargo run --release -p $PKG --bin $BIN -- --n-exits \$N --n-imported-exits \$N --proof-dir $LOG_DIR/proofs > $LOG_DIR/proof_${BIN}_\${N}.no_input.log 2>&1; if [ -n \"$INPUT_FILES\" ]; then IFS=',' read -ra FILES <<< \"$INPUT_FILES\"; for F in \"\${FILES[@]}\"; do BASE=\$(basename \"\$F\" | sed 's/\.[^.]*\$//'); echo \"[BASELINE] Running N=\$N INPUT=\$F\" | tee -a $LOG_DIR/run.log; /usr/bin/time -v cargo run --release -p $PKG --bin $BIN -- --n-exits \$N --n-imported-exits \$N --proof-dir $LOG_DIR/proofs --sample-path \$F > $LOG_DIR/proof_${BIN}_\${N}.input_\${BASE}.log 2>&1; done; fi; done" C-m
