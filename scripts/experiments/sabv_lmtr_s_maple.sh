...
################################################################################
# S-MAPLE BENCHMARK SCRIPT - Sequential Execution (like Baseline)
# 
...
LOG_DIR="/home/ubuntu/thanhhuyen/logs/s_maple_seq"
...
run_benchmark_sequential() {
    ...
    echo "🚀 Starting S-MAPLE Sequential Benchmark (lần lượt như baseline)"
    ...
    for N in "${N_VALUES[@]}"; do
        ...
        LOG_FILE="$LOG_DIR/s_maple_n${N}.log"
        START_TIME=$(date +%s)
        ...
        # S-MAPLE was using (4, 8) for N=500 → 64GB+ memory → OOM kill!
        # 
    ...
    echo ""
    echo "📊 Results:"
    echo "   Logs: $LOG_DIR/s_maple_n*.log"
...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "Chạy S-MAPLE benchmark TUẦN TỰ (lần lượt) - giống baseline"
    echo ""
    echo "Examples:"
