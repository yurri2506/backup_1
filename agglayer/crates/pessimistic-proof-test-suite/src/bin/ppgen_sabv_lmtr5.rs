...
use pessimistic_proof_core::{Lmtr4Algorithm, Lmtr4Config, Sabv5Algorithm, Sabv5Config};
...
struct PPGenArgs {
    ...
    /// Optional label to distinguish proof fixtures (e.g., round/timestamp)
    #[arg(long)]
...
fn calculate_optimal_height(n: usize, b: usize) -> usize {
    if n == 0 {
        return 1;
    }
    ((n as f64).ln() / (b as f64).ln()).ceil() as usize + 1
}
...
fn main() {
    ...
    info!(
        "🚀 Starting REAL SABV5/LMTR4 Proving with {} exits and {} validators",
    ...
    info!(
        "📦 Creating {} blocks (one per exit) for SABV5 verification",
        args.n_exits
    );
    ...
    for (i, (token_info, amount)) in all_bridge_exits.iter().enumerate() {
        ...
        let multi_batch_header = state_before
            ...
            .expect(&format!(
                "Failed to make multi batch header for block {}",
                i
            ));
    ...
    info!(
        "✅ Created {} blocks total for SABV5 verification",
        blocks.len()
    );

    let branching_factor = 3;
    let sabv5_temp = Sabv5Algorithm::new(Sabv5Config {
        branching_factor,
        num_validators: 5,
        secret_sharing_threshold: 3,
    });

    // Compute reference roots for fraud detection checks
    let (computed_root_from_original, _) = sabv5_temp
        .build_real_merkle_tree(&blocks, branching_factor)
        .expect("Failed to compute root from original blocks");
    info!(
        "📊 Computed root from {} original blocks (pre-LMTR reference): {:?}",
        blocks.len(),
        computed_root_from_original
    );

    // Simulate the blockchain root by applying the same LMTR4 rebalance strategy
    // that SABV5 uses internally before posting to L1.
    let target_height = calculate_optimal_height(blocks.len(), branching_factor);
    let mut lmtr4_sim = Lmtr4Algorithm::new(Lmtr4Config {
        branching_factor,
        target_height,
    });
    let simulated_rebalanced = lmtr4_sim
        .rebalance_local_cc_mbmt(&blocks)
        .expect("Failed to simulate LMTR4 rebalance for expected blockchain root");
    let (simulated_blockchain_root, _) = sabv5_temp
        .build_real_merkle_tree(&simulated_rebalanced.blocks, branching_factor)
        .expect("Failed to compute simulated blockchain root");
    info!(
        "🏛️ Simulated blockchain root after LMTR4 rebalance: {:?}",
        simulated_blockchain_root
    );
    ...
    let global_root = if let Some(wrong_root_hex) = &args.wrong_global_root {
        ...
        let wrong_root_bytes = hex::decode(hex_str).expect("Invalid wrong_global_root hex string");
        if wrong_root_bytes.len() != 32 {
            panic!(
                "wrong_global_root must be 32 bytes (64 hex characters), got {} bytes",
        ...
        // Normal mode: use simulated blockchain root (post-rebalance) for integrity check 3.
        simulated_blockchain_root
    };
    
    let setup_time = setup_start.elapsed();
    info!(
        "📊 Setup time (state + {} certificates): {:?}",
        blocks.len(),
        setup_time
    );
    ...
    if args.validator_nodes > 0 {
        info!(
            "🚀 Applying SABV5/LMTR4 REAL algorithms with {} validator nodes",
        ...
        info!(
            "⏱️  SABV5 REAL verification + rebalancing time: {:?}",
            sabv5_time
        );

        // **V5 FIX**: Handle fraud detection - allow continuation with flag
        if !integrity_verified {
            error!("❌ SABV5: Fraud detected! Check 3 (blockchain integrity) FAILED");
            error!("⚠️  SABV5 REAL integrity verification FAILED");
            error!("📝 This indicates data mismatch or tampering - fraud detected");
            error!("📝 Possible causes: Check 3 mismatch (simulation vs actual SABV5 partition/rebalance)");
            
            // DEBUG: Log flag status
            info!(
                "🔍 DEBUG: allow_fraud_testing flag = {}",
                args.allow_fraud_testing
            );
            ...
            if args.allow_fraud_testing {
                ...
                warn!("🔧 Check 3 failed but continuing with SP1 proving (testing mode)");
                ...
                error!("💡 Or restart script - it will auto-retry with --allow-fraud-testing");
                std::process::exit(1);
            }
        } else {
        ...
        info!(
            "📦 Using rebalanced blocks from SABV5 ({} blocks total)",
        ...
        info!(
            "📊 Baseline: {} bridge exits + {} imported bridge exits (total: {} events)",
            all_bridge_exits.len(),
            all_bridge_exits.len(),
            all_bridge_exits.len() * 2
        );
        ...
        info!(
            "🔄 Step 2: Applying {} bridge exits to create certificate...",
            all_bridge_exits.len()
        );
        ...
        info!(
            "✅ Step 2: Certificate created successfully with {} exits",
            all_bridge_exits.len()
        );
        let _ = std::io::stdout().flush();
        ...
        let enhanced_header = match initial_state.make_multi_batch_header(
            ...
            Err(e) => {
                ...
                let _ = std::io::stdout().flush();
                panic!(
                    "Failed to create final multi batch header for SP1 with {} exits: {:?}",
                    all_bridge_exits.len(),
                    e
                );
            }
        ...
        match proof_result {
            Ok((proof, vk, new_roots)) => {
                ...
                // Determine unique proof label (CLI-specified or timestamp fallback)
                let label = args
                    .run_label
                    .as_deref()
                    .map(|raw| raw.trim())
                ...
                info!("💾 Proof label: {}", label);
                
                // Save fixture
                let fixture_path = args
                    .proof_dir
                    .join(format!("v5_proof_n{}_{}.json", args.n_exits, label));
                ...
                std::fs::write(&fixture_path, fixture_json).expect("Failed to write fixture");
                
                let save_time = save_start.elapsed();
                info!(
                    "💾 Saved proof fixture to {} ({:?})",
                ...
                info!(
                    "   SABV5:            {:>12?} ({:>5.1}%)",
                    sabv5_time,
                    (sabv5_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0
                );
                info!(
                    "   SP1 Proving:      {:>12?} ({:>5.1}%)",
                    sp1_time,
                    (sp1_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0
                );
                info!(
                    "   Save fixture:     {:>12?} ({:>5.1}%)",
                    save_time,
                    (save_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0
                );
                info!("   ─────────────────────────────────");
                info!("   TOTAL:            {:>12?} (100.0%)", total_time);
                info!(
                    "📊 N={}, Memory usage will be shown by system (check dstat/top)",
                    args.n_exits
                );
            }
            Err(e) => {
