...
impl Sabv5Algorithm {
    ...
    pub fn verify_aggregated_blocks(
        ...
        (
            bool,
            Vec<
                crate::multi_batch_header::MultiBatchHeader<
                    agglayer_primitives::keccak::Keccak256Hasher,
                >,
            >,
        ),
    ...
    > {
        ...
        println!(
            "   Input: N={} blocks, branching_factor={}, validators={}",
            blocks.len(),
        ...
        println!(
            "🎯 V5: Calculated optimal htarget = {} for N={}, b={}",
            htarget, n, b
        );
        ...
        println!(
            "✅ Step 1: Optimal sharding size j* = {} (shard_size = {}^{} = {})",
            j_star,
            b,
            j_star,
            b.pow(j_star as u32)
        ...
        println!(
            "✅ Step 2: Partitioned {} blocks into {} shards (shard_size = {})",
            n,
            shards.len(),
            b.pow(j_star as u32)
        );
        ...
        println!(
            "✅ Step 3: Secret sharing applied (threshold k={}/{})",
        ...
        let rebalanced_shards: Result<Vec<_>, ProofError> = if n >= 50 {
            ...
            distributed_shards
                ...
                .map(|shard| {
                    ...
                    println!(
                        "   ✅ Shard {} (validator {}) - {} → {} blocks, height={}",
                        shard.shard_index,
                        shard.validator_id,
                        shard.blocks.len(),
                        rebalanced.blocks.len(),
                        rebalanced.height_after_rebalance
                    );
        ...
        let local_trees: Result<Vec<_>, ProofError> = if n >= 50 {
            rebalanced_shards
                ...
                println!(
                    "   ✅ Validator {} built REAL local CC-MBMT (shard {}) - {} blocks → tree height={}",
                    validator_id,
                    shard_idx,
                    rebalanced_blocks.len(),
                    tree_height
                );
        ...
        println!(
            "✅ Step 5b: Merged {} local CC-MBMTs into global root r",
            local_trees.len()
        );
        ...
        let (r_prime_from_rebalanced, r_prime_height) =
            self.build_real_merkle_tree(&all_rebalanced_blocks, b)?;
        println!("✅ Step 6: Computed alternative root r' (from rebalanced blocks) using SAME Merkle tree method as Step 5a");
        println!(
            "   Using {} rebalanced blocks, tree height={}",
            all_rebalanced_blocks.len(),
            r_prime_height
        );
        ...
        println!(
            "   r  (from merged local trees, rebalanced): {:?}",
            r_computed
        );
        println!(
            "   r' (from all rebalanced blocks):           {:?}",
            r_prime_from_rebalanced
        );
        println!(
            "   r' (from original blocks):                 {:?}",
            r_prime_from_original
        );
        println!(
            "   Note: r' (from original) will be compared with r (from rebalanced) in CHECK 2"
        );
        ...
        println!(
            "   ✅ Check 1: r == r' (from rebalanced blocks)? {} (internal consistency)",
            check1_roots_match
        );
        ...
        println!(
            "   📊 Block counts: original={}, rebalanced={}",
            original_block_count, rebalanced_block_count
        );
        ...
        let check2_rebalancing_match = if original_block_count != rebalanced_block_count {
            ...
            println!(
                "      • Unique blocks in rebalanced: {}",
                unique_rebalanced_hashes.len()
            );
            println!("      • Original blocks: {}", original_hashes.len());
            println!(
                "      • All original blocks present: {}",
                all_original_present
            );
        ...
        if !check1_roots_match {
            ...
            println!(
                "   ❌ Check 2: r' (from original blocks) ≠ r (from rebalanced blocks) - FAILED"
            );
            println!("   ⚠️  FRAUD DETECTED - rebalancing changed block content!");
            println!(
                "   ⚠️  Rebalancing should only change structure (add/remove blocks), not content"
            );
            println!(
                "   ⚠️  If roots don't match, blocks were modified during rebalancing (FRAUD)"
            );
            Ok((false, all_rebalanced_blocks))
        } else {
    ...
    fn choose_best_sharding_size(&self, n: usize, b: usize) -> Result<usize, ProofError> {
        ...
        println!(
            "📊 SABV5: Calculated j* = {} with variance = {:.4}",
            j_star, min_variance
        );
        Ok(j_star)
    }
    ...
    fn data_sharding(
        ...
        println!(
            "📦 SABV5: Created {} shards ({} full + {} remainder)",
            shards.len(),
            num_full_shards,
    ...
    fn receive_mpc_shares(&self, receiver_id: usize) -> Vec<(usize, (f64, f64))> {
        ...
        network
            .pending_messages
            .retain(|msg| msg.receiver_id != receiver_id);
    ...
    fn secret_sharing_distribution(
        &self,
        mut shards: Vec<ShardData>,
    ) -> Result<Vec<ShardData>, ProofError> {
        ...
        for (shard_idx, shard) in shards.iter_mut().enumerate() {
            ...
            let polynomial =
                self.generate_polynomial(secret_s, self.config.secret_sharing_threshold)?;
            ...
            for receiver_id in 0..self.config.num_validators {
                ...
                if received_shares.len() >= self.config.secret_sharing_threshold {
                    ...
                    let shares_for_lagrange: Vec<(usize, f64, f64)> = received_shares
                        .iter()
                        .map(|(id, (x, y))| (*id, *x, *y))
                        .collect();
                    let reconstructed_secret = self.reconstruct_secret_lagrange(
                        &shares_for_lagrange[..self.config.secret_sharing_threshold],
                    )?;
            ...
            let best_validator_id = if !reconstructed_secrets.is_empty() {
                let (id, _) = reconstructed_secrets
                    .iter()
            ...
            let reconstructed_secret = self.reconstruct_secret_lagrange(
                &validator_shares[..self.config.secret_sharing_threshold],
            )?;
            ...
            println!(
                "   Polynomial degree: {}, Shares: {}, Threshold: {}/{}",
                self.config.secret_sharing_threshold - 1,
                validator_shares.len(),
                self.config.secret_sharing_threshold,
                self.config.num_validators
            );
            println!(
                "   Validator {} assigned with share value: {:.6}",
                best_validator_id, validator_shares[best_validator_id].2
            );
        }
    ...
    fn construct_local_cc_mbmt(
        ...
        let local_trees_result: Result<Vec<_>, ProofError> = validator_shards
            ...
            .map(|(validator_id, validator_shard_list)| {
                ...
                println!(
                    "🌳 SABV5: Built local CC-MBMT for validator {} with {} blocks, height {}",
    ...
    fn merge_local_cc_mbmt(&self, local_trees: &[LocalCCMBMT]) -> Result<Digest, ProofError> {
        ...
        println!(
            "🔗 SABV5: Merged {} local CC-MBMTs into global CC-MBMT using REAL tree structure",
            local_trees.len()
        );
        Ok(global_root)
    }
    ...
    fn verify_integrity_with_blocks(
        ...
        expected_root: &agglayer_primitives::Digest,
        blocks: &[crate::multi_batch_header::MultiBatchHeader<
            agglayer_primitives::keccak::Keccak256Hasher,
        >],
        ...
        let signature_valid = if let (Some(sig), Some(signer_addr)) =
            (blockchain_signature, blockchain_signer)
        {
        ...
        println!(
            "🔍 SABV5: Blockchain signature verification - Valid: {}",
            signature_valid
        );
    ...
    fn validate_blockchain_signature(
        &self,
        signature: &[u8],
        public_key: &[u8],
    ) -> Result<bool, ProofError> {
        ...
        println!(
            "🔗 SABV5: Blockchain signature validation - Format: {}, Entropy: {:.3}, Valid: {}",
    ...
    fn verify_merkle_proof(
        ...
        let blockchain_proof =
            self.extract_blockchain_merkle_proof(computed_root, expected_root)?;
    ...
    fn extract_blockchain_merkle_proof(
        &self,
        computed_root: &Digest,
        expected_root: &agglayer_primitives::Digest,
    ) -> Result<BlockchainMerkleProof, ProofError> {
    ...
    fn verify_hash_chain(
        ...
        println!(
            "⛓️ SABV5: REAL hash chain verification - Rounds: 3, Entropy: {:.3}, Valid: {}",
    ...
    fn verify_timestamp_freshness(&self) -> Result<bool, ProofError> {
        ...
        println!(
            "⏰ SABV5: REAL timestamp verification - Block data has valid blockchain timestamps"
        );
    ...
    fn verify_zero_knowledge_proof(
        ...
        let completeness =
            commitment != challenge && challenge != response && response != commitment;
        let soundness = self.calculate_entropy(commitment.as_slice()) > 0.7;
        
        let zk_verified =
            commitment_valid && challenge_valid && response_valid && completeness && soundness;
        
        println!(
            "🔮 SABV5: REAL ZK proof verification - Complete: {}, Sound: {}, Verified: {}",
    ...
    fn generate_secret_from_shard(&self, shard: &ShardData) -> Result<f64, ProofError> {
        ...
        for block in &shard.blocks {
            let block_bytes =
                bincode::serialize(block).map_err(|_| ProofError::InvalidNullifierPath)?;
    ...
    fn compute_shard_hash(
        ...
        let hash_result = hasher.finalize();
        let hash_array: [u8; 32] = hash_result
            .as_slice()
            .try_into()
            .map_err(|_| ProofError::InvalidNullifierPath)?;
    ...
    fn calculate_optimal_height(&self, n: usize, b: usize) -> usize {
        ...
        println!(
            "   📐 Optimal height calculation: log_{}({}) + 1 = {}",
            b, n, optimal_height
        );
        optimal_height
    }
